...
def parse_args():
    ...
    # parser.add_argument('--config',default = '/opt/ml/baseline/mmdetection/configs/_base_/faster_rcnn_r50_fpn_1x_coco.py', help='train config file path')
    parser.add_argument('--config',default = '~/baseline/mmdetection/configs/_base_/faster_rcnn_r50_fpn_1x_coco.py', help='train config file path')
...
def main(fold, args ):
    ...
    cfg.seed = seed
    cfg.json_root = '/opt/ml/dataset/stratified_v3/'
    
    # train_kfold
    if args.no_validate: # 즉, no_validate를 하게 되면 validate안쓰면 무조건 all data로 학습시킨다는 의미.
        cfg.data.train.ann_file = '/opt/ml/dataset/train.json'
        fold = '_all_data'
